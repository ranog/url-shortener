...
async def test_it_should_return_an_empty_string_when_the_short_url_does_not_exist_in_the_repository(clean_collection):
    await clean_collection(COLLECTION_NAME)
    url_data = await UrlShortenerRepository().get_long_url('dummy_short_url')
    assert url_data == ''
